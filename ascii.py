...
char_list, font = get_data("complex", 13)
...
def AscifyImage(image, bg="black", res=-1, save=False, show=False):
    ...
    if save:
        cv2.imwrite("./Outputs/Out.jpg", output_bgr)
    if show:
        ShowImg(output_bgr)
    return output_bgr
...
def AsciifyVideo(video_path, background="black", frame_skip=1, resolution=-1):
    ...
    # Assembling Video from The Produced Frames
    in1 = ffmpeg.input(f'{path_frames}/frame_%d.jpg',
                       framerate=(vidCap.get(cv2.CAP_PROP_FPS)/frame_skip))
...
# AsciifyVideo('./data/NootNoot2D.webm',background="white", resolution = 500)
AscifyImage(cv2.imread(
    "/home/arpit/Pictures/Wallpapers/arch-linux-wallpaper-1080p.png"), save=True, show=False)
# AscifyImage(cv2.imread(
